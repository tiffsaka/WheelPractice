...
@IBDesignable open class CircleAnimatedMenu: UIControl {
    ...
    @IBInspectable public var innerRadius: CGFloat = 80 {
        didSet {
    ...
    @IBInspectable public var outerRadius: CGFloat = 150 {
        didSet {
    ...
    @IBInspectable public var titleFont: UIFont = UIFont.systemFont(ofSize: 20) {
        didSet {
            update()
        }
    }
    ...
    // Image size value
    public var imageSize: CGFloat = 40 {
        didSet {
            update()
        }
    }
    ...
    func updateUI() {
        ...
        for (index, value) in tuplesArray.enumerated() {
            ...
            sectionLayer.contentsGravity = CALayerContentsGravity.resizeAspect
        ...
        textLayer.frame = CGRect(x: center.x - innerRadius + 10,
                                 y: center.y - titleFont.pointSize,// / 2,
                                 width: 2 * innerRadius - 20,
                                 height: titleFont.pointSize + 6 + 20)
...
extension CircleAnimatedMenu: CAAnimationDelegate {
    ...
    public func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        ...
        if anim.value(forKey: "id") as! Int == sectionLayers.count - 1 {
            for sectionLayer in sectionLayers {
                let imageLayerIndex = sectionLayers.firstIndex(of: sectionLayer)
